    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine("Please enter amount storage: [TB]: ");
                string storage = Console.ReadLine();
                
                // TODO: do some data validation (optional)

                //string message = type + "@" + ram + "@" + cpuCount + "@" + storage;
                string message = $"{type}@{ram}@{cpuCount}@{storage}";      // gleiches wie oben

                // Daten losschicken: 
                client.Client.Send(Encoding.UTF8.GetBytes(message));        // Client damit man den Socket hat
                Console.WriteLine("----------------------");
            }
